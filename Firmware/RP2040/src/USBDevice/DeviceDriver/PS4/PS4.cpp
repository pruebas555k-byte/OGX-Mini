                                            ...
                                            uint8_t &out_x, uint8_t &out_y)
{
    ...
    if (curve != 1.0f) out_frac = std::pow(out_frac, curve);
...
void PS4Device::process(const uint8_t idx, Gamepad& gamepad)
{
    ...
    // STICKS NORMALES (sin curvas raras)
    ...
    apply_stick_steam_radial(gp_in.joystick_lx, gp_in.joystick_ly, left_deadzone, left_sensitivity, left_curve, report_in_.leftStickX, report_in_.leftStickY);
    apply_stick_steam_radial(gp_in.joystick_rx, gp_in.joystick_ry, right_deadzone, right_sensitivity, right_curve, report_in_.rightStickX, report_in_.rightStickY);
    ...
    if (physL2_val > 191) { virtR1 = true; }   // ← 75%
    ...
    if (time_reached(next_report_time) && tud_hid_ready())
    {
        ...
        next_report_time = make_timeout_time_ms(1);   // ← 1000 Hz (estable)
    }
}
...
const uint8_t* PS4Device::get_hid_descriptor_report_cb(uint8_t itf) { (void)itf; return PS4Dev::REPORT_DESCRIPTORS; }
const uint8_t* PS4Device::get_descriptor_configuration_cb(uint8_t index) { (void)index; return PS4Dev::CONFIGURATION_DESCRIPTORS; }
